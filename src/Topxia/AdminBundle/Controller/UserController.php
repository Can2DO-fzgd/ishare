...
class UserController extends BaseController 
{
    ...
    public function userNameCheckAction(Request $request)
    {
        ...
        list($result, $message) = $this->getAuthService()->checkUsername($userName);
        if ($result == 'success') {
            $response = array('success' => true, 'message' => '该注册手机可以使用');
        } else {
            $response = array('success' => false, 'message' => $message);
        }
        return $this->createJsonResponse($response);
    ...
    public function createAction(Request $request)
    {
        if ($request->getMethod() == 'POST') {
            ...
            $this->getLogService()->info('user', 'add', "管理员添加新用户 {$user['userName']} ({$user['id']})");
    ...
    public function editAction(Request $request, $id)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $this->getLogService()->info('user', 'edit', "管理员编辑用户资料 {$user['userName']} (#{$user['id']})", $profile);
    ...
    public function sendPasswordResetEmailAction(Request $request, $id)
    {
        ...
        try {
            $this->sendEmail(
                $user['email'],
                "重设{$user['userName']}在{$this->setting('site.name', 'MeBUS')}的密码",
            ...
            $this->getLogService()->error('user', 'send_password_reset', "管理员给用户 ${user['userName']}({$user['id']}) 发送密码重置邮件失败：". $e->getMessage());
            throw $e;
        }
    ...
    public function sendEmailVerifyEmailAction(Request $request, $id)
    {
        ...
        try {
            ...
            $this->getLogService()->error('user', 'send_email_verify', "管理员给用户 ${user['userName']}({$user['id']}) 发送Email验证邮件失败：". $e->getMessage());
            throw $e;
        }
