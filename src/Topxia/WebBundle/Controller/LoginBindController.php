...
class LoginBindController extends BaseController
{
    ...
    private function generateUser($type, $token, $oauthUser)
    {
        ...
        $userNames = array();
        $userNames[] = $oauthUser['name'];
        $userNames[] = mb_substr($oauthUser['name'], 0, 8, 'utf-8') . substr($randString, 0, 3);
        $userNames[] = mb_substr($oauthUser['name'], 0, 8, 'utf-8') . substr($randString, 3, 3);
        $userNames[] = mb_substr($oauthUser['name'], 0, 8, 'utf-8') . substr($randString, 6, 3);

        foreach ($userNames as $name) {
            if ($this->getUserService()->isUserNameAvaliable($name)) {
