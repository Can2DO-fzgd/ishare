...
class CourseThreadController extends BaseController
{
    public function indexAction(Request $request, $id)
    {
        ...
        if (!$this->getCourseService()->canTakeCourse($course)) {
            return $this->createMessageResponse('info', "您还不是产品《{$course['title']}》的购买用户，请先购买。", null, 3000, $this->generateUrl('course_show', array('id' => $id)));
        }
    ...
    public function showAction(Request $request, $courseId, $id)
    {
        ...
        if (!$this->getCourseService()->canTakeCourse($course)) {
            return $this->createMessageResponse('info', "您还不是产品《{$course['title']}》的购买会员，请先购买。", null, 3000, $this->generateUrl('course_show', array('id' => $courseId)));
        }
