...
class CourseStudentManageController extends BaseController
{
    ...
    public function createAction(Request $request, $id)
    {
        ...
        if ('POST' == $request->getMethod()) {
            ...
            $this->getLogService()->info('course', 'add_student', "产品《{$course['title']}》(#{$course['id']})，添加购买会员{$user['userName']}(#{$user['id']})，备注：{$data['remark']}");
    ...
    {   
        ...
        foreach ($courseMembers as $courseMember) {
            $member = "";
            $member .= $users[$courseMember['userId']]['userName'].",";
