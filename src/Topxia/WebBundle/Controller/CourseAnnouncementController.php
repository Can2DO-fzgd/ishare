	    ...
	    if($request->getMethod() == 'POST'){
        	...
        	if ($request->request->get('notify') == 'notify'){
	        	...
	        	foreach ($members as $member) {
		        	$result = $this->getNotificationService()->notify($member['userId'], 'default', "【产品公告】你正在关注的<a href='{$courseUrl}' target='_blank'>{$course['title']}</a>发布了一个新的公告，<a href='{$courseUrl}' target='_blank'>快去看看吧</a>");
	        	}
	        }
