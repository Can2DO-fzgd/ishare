...
class SettingsController extends BaseController
{
    ...
    public function userNameAction(Request $request)
    {
        ...
        if($is_userName['userName_enabled'] == 0){
            return $this->redirect($this->generateUrl('settings'));
        }
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $this->getAuthService()->changeUserName($user['id'], $userName);
            $this->setFlashMessage('success', '注册手机修改成功！');
            return $this->redirect($this->generateUrl('settings'));
        }
    ...
    public function userNameCheckAction(Request $request)
    {
        ...
        if ($currenUser['userName'] == $userName){
        ...
        if ($result == 'success'){
            ...
            $response = array('success' => false, 'message' => '注册手机已存在');
        }
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($form->isValid()) {
                ...
                try {
                    $this->sendEmail(
                        $data['email'],
                        "重设{$user['userName']}在" . $this->setting('site.name', 'MeBUS') . "的电子邮箱",
    ...
    public function emailVerifyAction()
    {
        ...
        try {
            $this->sendEmail(
                $user['email'],
                "验证{$user['userName']}在{$this->setting('site.name')}的电子邮箱",
    ...
    public function setupAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $this->getAuthService()->changeUserName($user['id'], $data['userName']);
    ...
    public function setupCheckUserNameAction(Request $request)
    {
        $user = $this->getCurrentUser();
