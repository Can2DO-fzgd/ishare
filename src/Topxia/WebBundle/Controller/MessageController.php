...
class MessageController extends BaseController
{
    ...
    public function createAction(Request $request, $toId)
    {
        ...
        $form = $this->createForm(new MessageType(), array('receiver'=>$receiver['userName']));
        if($request->getMethod() == 'POST'){
            $form->bind($request);
            if($form->isValid()){
                $message = $form->getData();
                $userName = $message['receiver'];
                $receiver = $this->getUserService()->getUserByUserName($userName);
                if(empty($receiver)) {
    ...
    public function matchAction(Request $request)
    {
        ...
        $findedUsersByUserName = $this->getUserService()->searchUsers(
            array('userName'=>$queryString),
            array('createdTime', 'DESC'),
            0,
            10);
        $findedFollowingIds = $this->getUserService()->filterFollowingIds($currentUser['id'], 
            ArrayToolkit::column($findedUsersByUserName, 'id'));
        ...
        foreach ($filterFollowingUsers as $filterFollowingUser) {
            $data[] = array(
                ...
                'userName' => $filterFollowingUser['userName']
            );
        }
