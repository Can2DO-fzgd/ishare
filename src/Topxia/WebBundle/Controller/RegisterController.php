...
class RegisterController extends BaseController
{
    ...
    private function  sendRegisterMessage($user)
    {
        ...
        $senderUser = $this->getUserService()->getUserByUserName($auth['welcome_sender']);
        if (empty($senderUser)) {
            return ;
        }
    ...
    private function getWelcomeBody($user)
    {
        ...
        $valuesToBeReplace = array('{{userName}}', '{{sitename}}', '{{siteurl}}');
        $valuesToReplace = array($user['userName'], $site['name'], $site['url']);
    ...
    private function sendVerifyEmail($token, $user)
    {
        ...
        $valuesToBeReplace = array('{{userName}}', '{{sitename}}', '{{siteurl}}', '{{verifyurl}}');
        $verifyurl = $this->generateUrl('register_email_verify', array('token' => $token), true);
        $valuesToReplace = array($user['userName'], $site['name'], $site['url'], $verifyurl);
