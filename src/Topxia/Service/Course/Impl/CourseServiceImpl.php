...
class CourseServiceImpl extends BaseService implements CourseService
{
	...
	public function updateLesson($courseId, $lessonId, $fields)
	{
		...
		$this->getLogService()->info('course', 'update_lesson', "更新产品介绍《{$lesson['title']}》({$lesson['id']})", $lesson);
	...
	public function setCourseTeachers($courseId, $teachers)
	{
		...
		foreach (array_values($teachers) as $index => $teacher) {
			if (empty($teacher['id'])) {
				throw $this->createServiceException("享客ID不能为空，设置产品(#{$courseId})享客失败");
			}
			$user = $this->getUserService()->getUser($teacher['id']);
			if (empty($user)) {
				throw $this->createServiceException("用户不存在或没有享客角色，设置产品(#{$courseId})享客失败");
			}
		...
		$this->getLogService()->info('course', 'update_teacher', "更新产品#{$courseId}的教师", $teacherMembers);
	...
	public function cancelTeacherInAllCourses($userId)
	{
		...
		$this->getLogService()->info('course', 'cancel_teachers_all', "取消用户#{$userId}所有的产品享客角色");
	}
    ...
    {
        $setting = $this->getSettingService()->get('course', array());
        $valuesToBeReplace = array('{{userName}}', '{{course}}');
        $valuesToReplace = array($user['userName'], $course['title']);
	...
	public function removeStudent($courseId, $userId)
	{
		...
		if (empty($member) or ($member['role'] != 'student')) {
			throw $this->createServiceException("用户(#{$userId})不是产品(#{$courseId})的会员，退出关注产品失败。");
		}
		...
		$this->getLogService()->info('course', 'remove_student', "产品《{$course['title']}》(#{$course['id']})，移除会员#{$member['id']}");
	}
	...
	public function lockStudent($courseId, $userId)
	{
		$course = $this->getCourse($courseId);
		if (empty($course)) {
			throw $this->createNotFoundException("产品(#${$courseId})不存在，封锁会员失败。");
		}

		$member = $this->getMemberDao()->getMemberByCourseIdAndUserId($courseId, $userId);
		if (empty($member) or ($member['role'] != 'student')) {
			throw $this->createServiceException("用户(#{$userId})不是产品(#{$courseId})的会员，封锁会员失败。");
		}
	...
	public function unlockStudent($courseId, $userId)
	{
		...
		$member = $this->getMemberDao()->getMemberByCourseIdAndUserId($courseId, $userId);
		if (empty($member) or ($member['role'] != 'student')) {
			throw $this->createServiceException("用户(#{$userId})不是产品(#{$courseId})的会员，解封会员失败。");
		}
