...
class UserServiceImpl extends BaseService implements UserService
{
    ...
    public function changeUserName ($userId, $userName)
    {
        ...
        if (!SimpleValidator::userName($userName)) {
            throw $this->createServiceException('用户注册手机格式不正确，设置帐号失败！');
        }
        $existUser = $this->getUserDao()->findUserByUserName($userName);
        if ($existUser && $existUser['id'] != $userId) {
            throw $this->createServiceException('注册手机已存在！');
        }
        $this->getLogService()->info('user', 'userName_change', "修改用户名{$user['userName']}为{$userName}成功");
        $this->getUserDao()->updateUser($userId, array('userName' => $userName));
    }
    ...
    public function promoteUser($id)
    {
        ...
        $this->getLogService()->info('user', 'recommend', "推荐用户{$user['userName']}(#{$user['id']})");
    }
    ...
    public function cancelPromoteUser($id)
    {
        ...
        $this->getLogService()->info('user', 'cancel_recommend', "取消推荐用户{$user['userName']}(#{$user['id']})");
    }
    ...
    public function passApproval($userId, $note = null)
    {
        ...
        $this->getLogService()->info('user', 'approved', "用户{$user['userName']}实名认证成功，操作人:{$currentUser['userName']} !" );
    ...
    public function rejectApproval($userId, $note = null)
    {
        ...
        $this->getLogService()->info('user', 'approval_fail', "用户{$user['userName']}实名认证失败，操作人:{$currentUser['userName']} !" );
