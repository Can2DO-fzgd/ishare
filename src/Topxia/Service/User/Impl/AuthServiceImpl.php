...
class AuthServiceImpl extends BaseService implements AuthService
{
    ...
    public function changeUserName($userId, $newName)
    {
        if ($this->hasPartnerAuth()) {
            ...
            $bind = $this->getUserService()->getUserBindByTypeAndUserId($providerName, $userId);
            if ($bind) {
                $this->getAuthProvider()->changeUserName($bind['fromId'], $newName);
            }
    ...
    public function checkUsername($username)
    {
        ...
        $avaliable = $this->getUserService()->isUserNameAvaliable($username);
        if (!$avaliable) {
            return array('error_duplicate', '此手机已经被注册了，换个手机号试试吧!');
        }
    ...
    public function checkPartnerLoginByUserName($userName, $password)
    {
        return $this->getAuthProvider()->checkLoginByUserName($userName, $password);
    }
