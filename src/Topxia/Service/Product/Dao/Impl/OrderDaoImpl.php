...
use Topxia\Service\Product\Dao\OrderDao;
use PDO;
...
class OrderDaoImpl extends BaseDao implements OrderDao
{
    ...
    public function sumOrderPriceByProductIdAndStatuses($productId, array $statuses)
    {
        ...
        $sql = "SELECT sum(price) FROM {$this->table} WHERE productId = ? AND status in ({$marks})";

        return $this->getConnection()->fetchColumn($sql, array_merge(array($productId), $statuses));
    }
