...
$twig->addGlobal('mebus_version', \Topxia\System::VERSION);
...
function _create_database($config, $replace)
{
	try {
		...
		$sql = file_get_contents('./mebus.sql');
		$result = $pdo->exec($sql);
		if ($result === false) {
...
EOD;
	    ...
	    $default = array(
	        ...
	        'welcome_sender' => $user['userName'],
	        'welcome_methods' => array(),
	        'welcome_title' => '欢迎加入{{sitename}}',
	        'welcome_body' => '您好{{userName}}，我是{{sitename}}的管理员，欢迎加入{{sitename}}，祝您购物愉快。如有问题，随时与我联系。',
	    );
